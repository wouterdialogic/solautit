...
$Button1 = GUICtrlCreateButton("Start", 105, 650, 32, 32, BitOR($BS_BITMAP, $BS_CENTER))
GUICtrlSetTip( -1, "Start")
ButtonImage($Button1, "media_play_green.png")
$Button2 = GUICtrlCreateButton("Stop", 165, 650, 32, 32, BitOR($BS_BITMAP, $BS_CENTER))
GUICtrlSetTip( -1, "Stop")
ButtonImage($Button2, "media_stop_red.png")
$Button3 = GUICtrlCreateButton("Pauze", 225, 650, 32, 32, BitOR($BS_BITMAP, $BS_CENTER))
GUICtrlSetTip( -1, "Pauze")
ButtonImage($Button3, "media_pause.png")
$Button4 = GUICtrlCreateButton("Test", 285, 650, 32, 32, BitOR($BS_BITMAP, $BS_CENTER))
GUICtrlSetTip( -1, "Test")
ButtonImage($Button4, "preferences_edit.png")
$Button5 = GUICtrlCreateButton("Standaard", 345, 650, 32, 32, BitOR($BS_BITMAP, $BS_CENTER))
GUICtrlSetTip( -1, "Standaard")
ButtonImage($Button5, "nav_undo_blue.png")
$Button6 = GUICtrlCreateButton("- i -", 350, 50, 32, 32, BitOR($BS_BITMAP, $BS_CENTER))
ButtonImage($Button6, "information.png")
GUICtrlSetTip( -1, "Help")
$Picture1 = GUICtrlSetImage(-1, @MyDocumentsDir & "\Batch\Settings\information.png",2 ,2)
$Line1 = GUICtrlCreateGraphic(20, 635, 400, 1)
GUICtrlSetBkColor(-1, 0xF0F0F0)
GUICtrlSetGraphic(-1, $GUI_GR_MOVE, 15) ; start point
GUICtrlSetGraphic(-1, $GUI_GR_COLOR, 0xA6A6A6)
GUICtrlSetGraphic(-1, $GUI_GR_LINE, 450)
...
Global $ItemType, $ItemInhoud, $sHoofdMenu, $sSubMenu, $sTab, $sSubTab, $sButton, $sTekst, $sActive, $sMaxTab, $svRecord, $svKeyLog, $vOmgeving, $vFilePath, $vFileName = ""
Global $iDelay = 250
...
Global $dragAndDropFileCounter = 0

;0x75407C [12]-Solidis logo test
;0x21608C [13]-Solidis logo branch

;ShellExecute(@MyDocumentsDir & "\Batch\1. Artikelen")
GUIRegisterMsg($WM_DROPFILES, "WM_DROPFILES_FUNC")

Func InitProg()
	ConsoleWrite(@ScriptLineNumber & ":" & "InitProg" & @HomeDrive & @CRLF)
	...
	$ExecuteVlag = 1
	$vYc = 230
	$Group2 = GUICtrlCreateGroup("Voortgang", 8, $vYc, 425, 400)
	$vYc = $vYc + 20
...
Func Pauze()
	...
	ToolTip("Script gepauzeerd, gebruik de F12 toets om script te hervatten!")
	$fPaused = Not $fPaused
	Local $i = 0
	Do
		If _IsPressed("7B") Then
			$i = $i + 1
			While _IsPressed("7B")
				ToolTip("F12 toets is ingedrukt")
				Sleep(10)
			WEnd
		EndIf
		Sleep(10)
...
Func ButtonImage($Button, $Image)
	Global $hImage = _GDIPlus_ImageLoadFromFile("D:\Documenten\Batch\Settings\Icons\" & $Image)
...
Func LogAVarAndMessage($variable, $message)
	ConsoleWrite($message & ": " & $variable & @CRLF)
EndFunc

Func RemoveLastProcessedFile($input)
	Local $aInput = StringSplit($input, "\")
	Local $lastElement = _ArrayPop($aInput)
	Local $secondLastElement = _ArrayPop($aInput)
	Local $aToString = _ArrayToString($aInput, "\", 1, -1)
	$aToString = $aToString & '\' & $lastElement

	LogAVarAndMessage($aToString, "$aToString")
...
Func VerwerkenBestand($f)
	...
	$vFilePath = StringSplit($sFilePath1, @MyDocumentsDir &  "\Batch\", $STR_ENTIRESPLIT)
	$vFileName = StringTrimRight($vFilePath[2], 4)
	LogAVarAndMessage($vFileName, "$vFileName")
	If $dragAndDropFileCounter > 0 Then
		;$vFileName = RemoveLastProcessedFile($vFileName, True)
		;LogAVarAndMessage($vFileName, "$vFileName after function")

		;LogAVarAndMessage($sFilePath1, "$sFilePath1")
		;$sFilePath1 = RemoveLastProcessedFile($sFilePath1, True)
		;LogAVarAndMessage($sFilePath1, "$sFilePath1 after function")
	EndIf
	$dragAndDropFileCounter = $dragAndDropFileCounter + 1
	;information, the first time the string is like this:
	;$sFilePath1: C:\Users\Test\Documents\Batch\Wouter\04_leveranciers\small steps\01_open_leverancier_menu.csv
	;
	;2nd
	;$sFilePath1: C:\Users\Test\Documents\Batch\Wouter\04_leveranciers\small steps\01_open_leverancier_menu.csv\02_maak_nieuwe_leverancier.csv
	;
	;so... remove everything from the right untill the \
	...
	$vXc = 320
	$vYc = $vYc + 20
	$Label10 = GUICtrlCreateLabel($vFileName, 18, $vYc, 350, 17)
...
Func Handleiding()
	ShellExecute("D:\Documenten\Batch\Settings\Handleiding testscripts.doc")
EndFunc
...
Func ReadQuery($f, $vFunction, $vLimit)
	...
	$vFilePath = StringSplit($sFilePath1, "D:\Documenten\Batch\", $STR_ENTIRESPLIT)
	$vFileName = StringTrimRight($vFilePath[2], 4)
	Local $sFilePath3 = "D:\Documenten\Batch\Input\" & $vFileName & ".txt"
...
Func WM_DROPFILES_FUNC($hWnd, $msgID, $wParam, $lParam)
	...
	For $i = 0 To $nAmt[0] - 1
		...
		ConsoleWrite("FileName: " & $gaDropFiles[$i] & @CRLF)
		$pFileName = 0
	Next
